...
func GetOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "an error occured while listing ordered item"})
			return
		}
...
func ItemsByOrder(id string) (OrderItems []primitive.M, err error) {
	...
	// manage the fields we are returning to the fron end
	// any field with a zero value is not propagated to the front end
	projectStage := bson.D{
		{
			"$project", bson.D{
				{"id", 0},
	...
	// group all the data according to a criteria
